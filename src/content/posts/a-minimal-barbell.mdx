...
I reselected thirty-five, forty-five, and—*wait a second.*
...
![](../../assets/posts/a-minimal-barbell/plates.jpeg)
...
Recall that this problem is known in computer science as "computationally difficult;" it is in fact [weakly NP-hard](https://en.wikipedia.org/wiki/Weak_NP-completeness). As the number of inputs (coin, or plate, denominations) increases, the time required to compute the optimal solution grows quickly.

But computational complexity doesn't give the final say as to whether a problem is worth solving in practice. Lots of day-to-day software searches through strings of text using the power of [regular expressions (with backreferences)](https://en.wikipedia.org/wiki/Regular_expression)—and that problem is NP-hard, too! We're working with a small number of inputs (and the change-making problem has a [pseudopolynomial-time](https://en.wikipedia.org/wiki/Pseudo-polynomial_time) algorithm, anyway), so the problem's [asymptotic](https://en.wikipedia.org/wiki/Asymptotic_computational_complexity) complexity is of little consequence.
...
<figure class="windowed" style="--aspect-ratio: 4 / 3">
	<p class="codepen" data-theme-id="dark" data-pen-title="combinatorial optimization for a minimal barbell" data-default-tab="result" data-slug-hash="xbwdNwQ" data-user="atsolanto" style="box-sizing: border-box; display: flex; align-items: center; justify-content: center; border: 2px solid; margin: 1em 0; padding: 1em;">
	<span>See the Pen <a href="https://codepen.io/atsolanto/pen/xbwdNwQ">
	combinatorial optimization for a minimal barbell</a> by Andrew (<a href="https://codepen.io/atsolanto">@atsolanto</a>)
